 ...
 * - launching AlgoScore and X11 when double-clicked 
 * - bringing X11 to the top when its icon is clicked in the dock (via a small applescript)
 * - catch file dropped on icon events (and double-clicked gimp documents) and notify AS.
 * - catch quit events performed outside AS, e.g. on the dock icon.
 */
...
static void ShowFirstStartWarningDialog(void)
{
    ...
    StandardAlert(kAlertNoteAlert, "\pAlgoScore on Mac OS X",
            "\pWhile AlgoScore is open, its windows can be displayed or hidden by displaying or hiding the X11 application.\n\nThe first time this version of AlgoScore is run it may take several minutes before the main window is displayed while font caches are built.",
            &params, &itemHit);
}
                                 ...
                                 EventRef theEvent, void *userData)
{
    ...
    system("test -d \"$HOME/.algoscore-etc\" || mkdir \"$HOME/.algoscore-etc\"; "
           "touch \"$HOME/.algoscore-etc/.fccache-new\"");
    // Rerun now.
                                 ...
                                 EventRef theEvent, void *userData)
{
	...
	StandardAlert(kAlertStopAlert, "\pFailed to start X11",
			"\pAlgoScore.app requires Apple's X11, which is freely downloadable from Apple's website for Panther (10.3.x) users and available as an optional install from the installation DVD for Tiger (10.4.x) users.\n\nPlease install X11 and restart AlgoScore.",
			&params, &itemHit);
