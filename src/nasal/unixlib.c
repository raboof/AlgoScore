...
static naRef f_unlink(naContext ctx, naRef me, int argc, naRef* args)
{
    char *fn = argc > 0 ? naStr_data(naStringValue(ctx, args[0])) : 0;
    int res = 0;
    if(fn) res = unlink(fn);
    return naNum(res);
}

static naRef f_readlink(naContext ctx, naRef me, int argc, naRef* args)
{
    naRef f = argc > 0 ? naStringValue(ctx, args[0]) : naNil();
    char s[1024];
    memset(s,0,1024);
    if(!IS_STR(f))
        naRuntimeError(ctx,"bad argument to readlink");
    if(readlink(naStr_data(f),s,1023) < 0) {
        if(errno == ENOENT) return naNil();
        if(errno == EINVAL) return f;
        naRuntimeError(ctx, strerror(errno));
    }
    return NEWCSTR(ctx, s);
}

static naRef f_getpid(naContext ctx, naRef me, int argc, naRef* args)
{
    return naNum(getpid());
}

static naRef f_mkdir(naContext ctx, naRef me, int argc, naRef* args)
{
    char *fn = argc > 0 ? naStr_data(naStringValue(ctx, args[0])) : 0;
    mode_t mode = argc > 1 ? naNumValue(args[1]).num : 0777;
    int res = 0;
    if(fn) res = mkdir(fn, mode);
    return naNum(res);
}
